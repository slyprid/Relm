...
namespace Relm.Components.Renderables.Particles
{
        ...
        : RenderableComponent, IUpdateable
	{
		...
		public ParticleEmitter(ParticleEmitterConfig emitterConfig, bool playOnAwake = true)
		{
			...
			CollisionConfig.CollidesWithLayers = RelmPhysics.AllLayers;
		...
		public override void Render(Graphics.SpriteBatch spriteBatch, Camera camera)
		{
			...
			for (var i = 0; i < _particles.Count; i++)
			{
				...
				if (_emitterConfig.Sprite == null)
                    spriteBatch.Draw(RelmGraphics.Instance.PixelTexture, pos + currentParticle.position, currentParticle.color,
						currentParticle.rotation, Vector2.One, currentParticle.particleSize * 0.5f, SpriteEffects.None,
						LayerDepth);
				else
                    spriteBatch.Draw(_emitterConfig.Sprite, pos + currentParticle.position,
