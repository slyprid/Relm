...
{
	public class SpriteTrail : RenderableComponent, IUpdateable
	{
		/// <summary>
		...
		class SpriteTrailInstance
		{
			...
			public bool Update()
			{
				...
				if (_elapsedTime > _fadeDelay && _elapsedTime < _fadeDuration + _fadeDelay)
				{
					...
					ColorExtensions.Lerp(ref _initialColor, ref _targetColor, out _renderColor, t);
				}
			...
			{
                spriteBatch.Draw(_sprite, Position, _renderColor, _rotation, _origin, _scale, _spriteEffects,
					_layerDepth);
			}
		}
		...
		public virtual void Update()
		{
			...
			{
				var distanceMoved = System.Math.Abs(Vector2.Distance(Entity.Transform.Position + _localOffset, _lastPosition));
		...
		{
			for (var i = 0; i < _liveSpriteTrailInstances.Count; i++)
				_liveSpriteTrailInstances[i].Render(spriteBatch, camera);
		}
	}
}