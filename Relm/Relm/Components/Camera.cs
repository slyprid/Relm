	...
	public class Camera : Component
	{
		...
		public RectangleF Bounds
		{
			get
			{
				...
				if (_areBoundsDirty)
				{
					...
					var topLeft = ScreenToWorldPoint(new Vector2(RelmGame.GraphicsDevice.Viewport.X + _inset.left,
						RelmGame.GraphicsDevice.Viewport.Y + _inset.top));
					var bottomRight = ScreenToWorldPoint(new Vector2(
						RelmGame.GraphicsDevice.Viewport.X + RelmGame.GraphicsDevice.Viewport.Width - _inset.right,
						RelmGame.GraphicsDevice.Viewport.Y + RelmGame.GraphicsDevice.Viewport.Height - _inset.bottom));
					...
					if (Entity.Transform.Rotation != 0)
					{
						...
						var topRight = ScreenToWorldPoint(new Vector2(
							RelmGame.GraphicsDevice.Viewport.X + RelmGame.GraphicsDevice.Viewport.Width - _inset.right,
							RelmGame.GraphicsDevice.Viewport.Y + _inset.top));
						var bottomLeft = ScreenToWorldPoint(new Vector2(RelmGame.GraphicsDevice.Viewport.X + _inset.left,
							RelmGame.GraphicsDevice.Viewport.Y + RelmGame.GraphicsDevice.Viewport.Height - _inset.bottom));
		...
		public Matrix ProjectionMatrix
		{
			get
			{
				...
				{
					Matrix.CreateOrthographicOffCenter(0, RelmGame.GraphicsDevice.Viewport.Width,
						RelmGame.GraphicsDevice.Viewport.Height, 0, 0, -1, out _projectionMatrix);
		...
		public Matrix ProjectionMatrix3D
		{
			get
			{
				var targetHeight = (RelmGame.GraphicsDevice.Viewport.Height / _zoom);
				var fov = (float)System.Math.Atan(targetHeight / (2f * PositionZ3D)) * 2f;
				return Matrix.CreatePerspectiveFieldOfView(fov, RelmGame.GraphicsDevice.Viewport.AspectRatio,
		...
		{
			UpdateMatrixes();
			Vector2Extensions.Transform(ref worldPosition, ref _transformMatrix, out worldPosition);
		...
		{
			UpdateMatrixes();
            Vector2Extensions.Transform(ref screenPosition, ref _inverseTransformMatrix, out screenPosition);
		...
		{
			return ScreenToWorldPoint(RelmInput.MousePosition);
		}
