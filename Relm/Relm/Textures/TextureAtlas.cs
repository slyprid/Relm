...
namespace Relm.Textures
{
    public class TextureAtlas
        : IEnumerable<TextureRegion2D>
    {
        ...
        public int RegionCount => _regionsByName.Count;
        ...
        private bool ContainsRegion(string name)
        {
        ...
        {
            if (_regionsByName.ContainsKey(name)) throw new InvalidOperationException($"Region {name} already exists in the texture atlas");
        ...
        {
            if (!_regionsByName.TryGetValue(name, out var region)) return;
        ...
        public static TextureAtlas Create(string name, Texture2D texture, int regionWidth, int regionHeight, int maxRegionCount = int.MaxValue, int margin = 0, int spacing = 0)
        {
