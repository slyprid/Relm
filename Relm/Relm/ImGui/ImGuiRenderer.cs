...
namespace Relm.Gui
{
	public class ImGuiRenderer
	{
		...
		public unsafe void RebuildFontAtlas(ImGuiOptions options)
		{
			...
			var tex2d = new Texture2D(RelmGame.GraphicsDevice, width, height, false, SurfaceFormat.Color);
 ...
 Marshal.GetFunctionPointerForDelegate<GetClipboardTextDelegate>( SDL2.SDL.SDL_GetClipboardText );
...
#if !FNA
			RelmGame.Instance.Window.TextInput += (s, a) =>
			{
				if (a.Character == '\t')
					return;
		...
		{
			_effect = _effect ?? new BasicEffect(RelmGame.GraphicsDevice);
		...
		void UpdateInput()
		{
			...
			var mouse = RelmInput.CurrentMouseState;
			var keyboard = RelmInput.CurrentKeyboardState;
			...
			io.DisplaySize = new System.Numerics.Vector2(RelmGame.GraphicsDevice.PresentationParameters.BackBufferWidth,
				RelmGame.GraphicsDevice.PresentationParameters.BackBufferHeight);
		...
		void RenderDrawData(ImDrawDataPtr drawData)
		{
			...
			var lastViewport = RelmGame.GraphicsDevice.Viewport;
			var lastScissorBox = RelmGame.GraphicsDevice.ScissorRectangle;

			RelmGame.GraphicsDevice.BlendFactor = Color.White;
			RelmGame.GraphicsDevice.BlendState = BlendState.NonPremultiplied;
			RelmGame.GraphicsDevice.RasterizerState = _rasterizerState;
			RelmGame.GraphicsDevice.DepthStencilState = DepthStencilState.DepthRead;

			// Handle cases of screen coordinates != from framebuffer coordinates (e.g. retina displays)
			drawData.ScaleClipRects(ImGui.GetIO().DisplayFramebufferScale);

			// Setup projection
			RelmGame.GraphicsDevice.Viewport = new Viewport(0, 0,
				RelmGame.GraphicsDevice.PresentationParameters.BackBufferWidth,
				RelmGame.GraphicsDevice.PresentationParameters.BackBufferHeight);
			...
			RelmGame.GraphicsDevice.Viewport = lastViewport;
			RelmGame.GraphicsDevice.ScissorRectangle = lastScissorBox;
		}
		...
		unsafe void UpdateBuffers(ImDrawDataPtr drawData)
		{
			...
			if (drawData.TotalVtxCount > _vertexBufferSize)
			{
				...
				_vertexBuffer = new VertexBuffer(RelmGame.GraphicsDevice, _vertexDeclaration, _vertexBufferSize,
					BufferUsage.None);
			...
			if (drawData.TotalIdxCount > _indexBufferSize)
			{
				...
				_indexBuffer = new IndexBuffer(RelmGame.GraphicsDevice, IndexElementSize.SixteenBits, _indexBufferSize,
					BufferUsage.None);
		...
		{
			RelmGame.GraphicsDevice.SetVertexBuffer(_vertexBuffer);
			RelmGame.GraphicsDevice.Indices = _indexBuffer;
			...
			for (int n = 0; n < drawData.CmdListsCount; n++)
			{
				...
				for (int cmdi = 0; cmdi < cmdList.CmdBuffer.Size; cmdi++)
				{
					...
					RelmGame.GraphicsDevice.ScissorRectangle = new Rectangle(
...
#pragma warning disable CS0618 // FNA does not expose an alternative method.
						RelmGame.GraphicsDevice.DrawIndexedPrimitives(
