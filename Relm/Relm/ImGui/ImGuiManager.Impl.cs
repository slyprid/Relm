...
using Relm.Persistence.Binary.DataStore;
using Relm.Scenes;
using Relm.Utilities;
...
namespace Relm.Gui
{
	public partial class ImGuiManager : GlobalManager, IFinalRenderDelegate, IDisposable
	{
		...
		void LoadSettings()
		{
			var fileDataStore = RelmGame.Services.GetService<FileDataStore>() ?? new FileDataStore(Storage.GetStorageRoot());
			...
			RelmGame.Emitter.AddObserver(CoreEvents.Exiting, PersistSettings);
		}
		...
		void PersistSettings()
		{
			...
			KeyValueDataStore.Default.Flush(RelmGame.Services.GetOrAddService<FileDataStore>());
		}
			...
			if (!ImGui.IsWindowFocused())
			{
				...
				if (!focusedWindow)
				{
					var mouseState = new MouseState(
						RelmInput.CurrentMouseState.X,
						RelmInput.CurrentMouseState.Y,
						DisableMouseWheelWhenGameWindowUnfocused ? 0 : RelmInput.MouseWheel,
                    ...
                    RelmInput.SetCurrentMouseState(mouseState);

					if (DisableKeyboardInputWhenGameWindowUnfocused)
					{
                        RelmInput.SetCurrentKeyboardState(new KeyboardState());
					}
				}
			}
		...
		void OverrideMouseInput()
		{
			...
			var normalizedPos = RelmInput.RawMousePosition.ToVector2() - offset;
			...
			var unNormalizedPos = normalizedPos / RelmInput.ResolutionScale;
			unNormalizedPos += RelmInput.ResolutionOffset;

			var mouseState = RelmInput.CurrentMouseState;
			var newMouseState = new MouseState((int)unNormalizedPos.X, (int)unNormalizedPos.Y,
				...
				mouseState.XButton2);
            RelmInput.SetCurrentMouseState(newMouseState);
		}
		...
		{
			if (RelmGame.Scene != null)
			{
				RelmGame.Scene.FinalRenderDelegate = this;
		...
		public override void OnDisabled()
		{
			Unload();
			if (RelmGame.Scene != null)
				RelmGame.Scene.FinalRenderDelegate = null;
		}
													...
													SamplerState samplerState)
		{
			if (ShowSeperateGameWindow)
			{
				...
				RelmGame.GraphicsDevice.SamplerStates[0] = samplerState;
			...
			else
			{
				...
				RelmGraphics.Instance.SpriteBatch.Draw(source, finalRenderDestinationRect, Color.White);
		...
		void Dispose(bool disposing)
		{
			if (!_isDisposed)
			{
				if (disposing)
				{
					RelmGame.Emitter.RemoveObserver(CoreEvents.SceneChanged, OnSceneChanged);
				}
		...
		public static void ToggleImGui()
		{
			...
			var service = RelmGame.GetGlobalManager<ImGuiManager>();
			if (service == null)
			{
