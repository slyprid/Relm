...
using Relm.Gui.Inspectors;
using Relm.Gui.Tools;
...
namespace Relm.Gui
{
	public partial class ImGuiManager : GlobalManager, IFinalRenderDelegate, IDisposable
	{
		...
		public ImGuiManager(ImGuiOptions options = null)
		{
			...
			LoadSettings();
			_renderer = new ImGuiRenderer(RelmGame.Instance);

			_renderer.RebuildFontAtlas(options);
			RelmGame.Emitter.AddObserver(CoreEvents.SceneChanged, OnSceneChanged);
			RelmImGuiThemes.DarkTheme1();

			// find all Scenes
			_sceneSubclasses = Relm.Utils.GetAllSubclasses(typeof(Scene), true);
			...
			// find all themes
			_themes = typeof(RelmImGuiThemes).GetMethods(System.Reflection.BindingFlags.Static |
		...
		void DrawMainMenuBar()
		{
			if (ImGui.BeginMainMenuBar())
			{
				...
				if (_sceneSubclasses.Count > 0 && ImGui.BeginMenu("Scenes"))
				{
					foreach (var sceneType in _sceneSubclasses)
					{
						if (ImGui.MenuItem(sceneType.Name))
						{
							...
							RelmGame.StartSceneTransition(new FadeTransition(() => scene));
						}
					}
				...
				{
					var rtSize = RelmGame.Scene.SceneRenderTargetSize;
