	...
	public class SpriteAtlasEditorWindow
	{
		...
		void LoadTextureAndAtlasFiles()
		{
			if (_texturePtr != IntPtr.Zero)
				RelmGame.GetGlobalManager<ImGuiManager>().UnbindTexture(_texturePtr);
			...
			var _atlasTexture = Texture2D.FromStream(RelmGame.GraphicsDevice, File.OpenRead(_sourceImageFile));
			_textureSize = new Num.Vector2(_atlasTexture.Width, _atlasTexture.Height);
			_textureAspectRatio = _textureSize.X / _textureSize.Y;
			_texturePtr = RelmGame.GetGlobalManager<ImGuiManager>().BindTexture(_atlasTexture);
		...
		void DrawLeftPane()
		{
			...
			if (RelmImGui.CenteredButton("Set All Origins", 0.75f))
			{
		...
		void DrawCenterPane()
		{
			if (ImGui.IsWindowFocused() && ImGui.GetIO().MouseWheel != 0)
			{
				...
				_imageZoom += System.Math.Min(maxZoom - _imageZoom, ImGui.GetIO().MouseWheel * zoomSpeed);
		...
		void DrawRightPane()
		{
			...
			if (RelmImGui.CenteredButton("Add Animation", 0.5f))
			...
			if (ImGui.BeginPopup("add-animation"))
			{
				...
				{
					_stringBuffer = _stringBuffer.Length > 0 ? _stringBuffer : Relm.Utils.RandomString(8);
		...
		void GenerateRects(int cellWidth, int cellHeight, int totalFrames, int padding, int cellOffset = 0)
		{
			...
			for (var y = 0; y < rows; y++)
			{
				for (var x = 0; x < cols; x++)
				{
					...
					_spriteAtlasData.Origins.Add(Vector2Extensions.HalfVector());
