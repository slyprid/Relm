...
namespace Relm.Gui.Inspectors.SceneGraphPanes
{
	public class EntityPane
	{
		...
		{
			if (RelmGame.Scene.Entities.Count > MIN_ENTITIES_FOR_CLIPPER)
			{
				var clipperPtr = ImGuiNative.ImGuiListClipper_ImGuiListClipper(RelmGame.Scene.Entities.Count, -1);
				var clipper = new ImGuiListClipperPtr(clipperPtr);

				while (clipper.Step())
					for (var i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
						DrawEntity(RelmGame.Scene.Entities[i]);

				ImGuiNative.ImGuiListClipper_destroy(clipperPtr);
			}
			else
			{
				for (var i = 0; i < RelmGame.Scene.Entities.Count; i++)
					DrawEntity(RelmGame.Scene.Entities[i]);
			}

            RelmImGui.MediumVerticalSpace();
			if (RelmImGui.CenteredButton("Create Entity", 0.6f))
			{
		...
		void DrawEntityContextMenuPopup(Entity entity)
		{
			if (ImGui.BeginPopup("entityContextMenu"))
			{
				...
				{
					var clone = entity.Clone(RelmGame.Scene.Camera.Position);
				...
				if (ImGui.BeginPopup("create-new-entity"))
				{
					...
					{
						_newEntityName = _newEntityName.Length > 0 ? _newEntityName : Relm.Utils.RandomString(8);
		...
		void DrawCreateEntityPopup()
		{
			if (ImGui.BeginPopup("create-entity"))
			{
				...
				if (ImGui.Button("Create"))
				{
					_newEntityName = _newEntityName.Length > 0 ? _newEntityName : Relm.Utils.RandomString(8);
					var newEntity = new Entity(_newEntityName);
					newEntity.Transform.Position = RelmGame.Scene.Camera.Transform.Position;
                    RelmGame.Scene.AddEntity(newEntity);
