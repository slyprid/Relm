...
namespace Relm.Assets.BitmapFonts
{
	public partial class BitmapFont
	{
		...
		public void DrawInto(SpriteBatch spriteBatch, string text, Vector2 position, Color color, float rotation, Vector2 origin, Vector2 scale, SpriteEffects effect, float depth)
		{
			var source = new FontCharacterSource(text);
			DrawInto(spriteBatch, ref source, position, color, rotation, origin, scale, effect, depth);
		}

		public void DrawInto(SpriteBatch spriteBatch, StringBuilder text, Vector2 position, Color color, float rotation, Vector2 origin, Vector2 scale, SpriteEffects effect, float depth)
		{
			var source = new FontCharacterSource(text);
			DrawInto(spriteBatch, ref source, position, color, rotation, origin, scale, effect, depth);
		}
		...
		public void DrawInto(SpriteBatch spriteBatch, ref FontCharacterSource text, Vector2 position, Color color, float rotation, Vector2 origin, Vector2 scale, SpriteEffects effect, float depth)
		{
			...
			for (var i = 0; i < text.Length; ++i)
			{
				...
				var destRect = RectangleExtensions.FromFloats
				(
					p.X, p.Y,
                ...
                spriteBatch.Draw(Textures[currentChar.TexturePage], destRect, currentChar.Bounds, color, rotation, Vector2.Zero, effect, depth);
				previousCharacter = c;
			}
		}
	}
