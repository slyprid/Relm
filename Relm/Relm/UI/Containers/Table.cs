	...
	public class Table : Group
	{
		...
		public override float PreferredWidth
		{
			get
			{
				...
				if (_background != null)
					return System.Math.Max(width, _background.MinWidth);
		...
		public override float PreferredHeight
		{
			get
			{
				...
				if (_background != null)
					return System.Math.Max(height, _background.MinHeight);
		...
		public override void Draw(SpriteBatch spriteBatch, float parentAlpha)
		{
			...
			if (transform)
			{
				...
				DrawBackground(spriteBatch, parentAlpha, 0, 0);

				if (Clip)
				{
                    spriteBatch.FlushBatch();
					float padLeft = _padLeft.Get(this), padBottom = _padBottom.Get(this);
					if (ClipBegin(spriteBatch, padLeft, padBottom, GetWidth() - padLeft - _padRight.Get(this),
			...
			{
				DrawBackground(spriteBatch, parentAlpha, x, y);
		...
		/// drawable.
		/// </summary>
		protected virtual void DrawBackground(SpriteBatch spriteBatch, float parentAlpha, float x, float y)
		{
			if (_background == null)
				return;

			_background.Draw(spriteBatch, x, y, width, height, ColorExtensions.Create(color, (int)(color.A * parentAlpha)));
		}
		...
		void ComputeSize()
		{
			...
			_tablePrefWidth = System.Math.Max(_tablePrefWidth + hpadding_, _tableMinWidth);
			_tablePrefHeight = System.Math.Max(_tablePrefHeight + vpadding_, _tableMinHeight);
		}
		...
		public override void DebugRender(SpriteBatch spriteBatch)
		{
			if (_debugRects != null)
			{
				foreach (var d in _debugRects)
					spriteBatch.DrawHollowRect(x + d.Rect.X, y + d.Rect.Y, d.Rect.Width, d.Rect.Height, d.Color);
			}

			base.DebugRender(spriteBatch);
		}
