...
namespace Relm.UI.Containers
{
	public class SplitPane : Group, IInputListener
	{
		...
		public override void Draw(SpriteBatch spriteBatch, float parentAlpha)
		{
			...
			{
				var scissor = ScissorStack.CalculateScissors(_stage?.Camera, spriteBatch.TransformMatrix, _firstWidgetBounds);
				if (ScissorStack.PushScissors(scissor))
				{
					spriteBatch.EnableScissorTest(true);
					_firstWidget.Draw(spriteBatch, parentAlpha * color.A);
			...
			{
				var scissor = ScissorStack.CalculateScissors(_stage?.Camera, spriteBatch.TransformMatrix,
					_secondWidgetBounds);
				if (ScissorStack.PushScissors(scissor))
				{
					spriteBatch.EnableScissorTest(true);
					_secondWidget.Draw(spriteBatch, parentAlpha * color.A);
					spriteBatch.EnableScissorTest(false);
					ScissorStack.PopScissors();
				}
			}

			_style.Handle.Draw(spriteBatch, _handleBounds.X, _handleBounds.Y, _handleBounds.Width, _handleBounds.Height,
				ColorExtensions.Create(color, (int)(color.A * parentAlpha)));
