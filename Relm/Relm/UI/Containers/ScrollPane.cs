...
namespace Relm.UI.Containers
{
	public class ScrollPane : Group, IInputListener
	{
		...
		public override void Layout()
		{
			...
			_widgetAreaBounds = RectangleExtensions.FromFloats(bgLeftWidth, bgBottomHeight, _areaWidth, _areaHeight);
			...
			widgetWidth = _disableX ? _areaWidth : System.Math.Max(_areaWidth, widgetWidth);
			widgetHeight = _disableY ? _areaHeight : System.Math.Max(_areaHeight, widgetHeight);
			...
			if (_scrollX)
			{
				if (hScrollKnob != null)
				{
					...
					_hScrollBounds = RectangleExtensions.FromFloats(boundsX, boundsY, _areaWidth, hScrollHeight);
			...
			if (_scrollY)
			{
				if (vScrollKnob != null)
				{
					...
					_vScrollBounds = RectangleExtensions.FromFloats(boundsX, boundsY, vScrollWidth, _areaHeight);
		...
		public override void Draw(SpriteBatch spriteBatch, float parentAlpha)
		{
			...
			if (_style.Background != null)
				_style.Background.Draw(spriteBatch, 0, 0, GetWidth(), GetHeight(), color);

			// caculate the scissor bounds based on the batch transform, the available widget area and the camera transform. We need to
			// project those to screen coordinates for OpenGL to consume.
			var scissor =
				ScissorStack.CalculateScissors(_stage?.Camera, spriteBatch.TransformMatrix, _widgetAreaBounds);
			...
			if (_scrollX && _scrollY)
			{
				if (_style.Corner != null)
					_style.Corner.Draw(spriteBatch, _hScrollBounds.X + _hScrollBounds.Width, _hScrollBounds.Y,
			...
			{
				if (_style.HScroll != null)
					_style.HScroll.Draw(spriteBatch, _hScrollBounds.X, _hScrollBounds.Y, _hScrollBounds.Width,
						_hScrollBounds.Height, color);
				if (_style.HScrollKnob != null)
					_style.HScrollKnob.Draw(spriteBatch, _hKnobBounds.X, _hKnobBounds.Y, _hKnobBounds.Width,
			...
			if (_scrollY)
			{
				if (_style.VScroll != null)
					_style.VScroll.Draw(spriteBatch, _vScrollBounds.X, _vScrollBounds.Y, _vScrollBounds.Width,
						_vScrollBounds.Height, color);
				if (_style.VScrollKnob != null)
					_style.VScrollKnob.Draw(spriteBatch, _vKnobBounds.X, _vKnobBounds.Y, _vKnobBounds.Width,
		...
		public override void DebugRender(SpriteBatch spriteBatch)
		{
			...
			{
				spriteBatch.EnableScissorTest(true);
				DebugRenderChildren(spriteBatch, 1f);
				spriteBatch.EnableScissorTest(false);
				ScissorStack.PopScissors();
			}
