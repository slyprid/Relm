...
namespace Relm.UI
{
	public class Skin
	{
		...
		public T SetStylesForStyleClass<T>(T styleClass, Dictionary<string, object> styleDict, RelmContentManager contentManager, string styleName)
		{
			foreach (var styleConfig in styleDict)
			{
				...
				{
					Relm.Utils.GetFieldInfo(styleClass, name).SetValue(styleClass, GetColor(identifier));
				}
				else if (name.ToLower().Contains("font"))
				{
                    Relm.Utils.GetFieldInfo(styleClass, name)
						.SetValue(styleClass, contentManager.Load<BitmapFont>(identifier));
				}
				else if (name.ToLower().EndsWith("style"))
				{
					var styleField = Relm.Utils.GetFieldInfo(styleClass, name);
					...
					if (valueObject is Dictionary<string, object>)
					{
						...
						var setStylesForStyleClassMethod =
                            Relm.Utils.GetMethodInfo(this, "SetStylesForStyleClass");
					...
					else
					{
						...
						var getStyleMethod = Relm.Utils.GetMethodInfo(this, "Get", new Type[] { typeof(string) });
				...
				else
				{
					...
					if (drawable != null)
                        Relm.Utils.GetFieldInfo(styleClass, name).SetValue(styleClass, drawable);
					else
