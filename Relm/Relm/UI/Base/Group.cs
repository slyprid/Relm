...
namespace Relm.UI.Base
{
	public class Group : Element, ICullable
	{
		...
		public void DebugRenderChildren(SpriteBatch spriteBatch, float parentAlpha)
		{
			parentAlpha *= color.A / 255.0f;
			if (transform)
			{
				for (var i = 0; i < children.Count; i++)
				{
					...
					children[i].DebugRender(spriteBatch);
				}
			}
			else
			{
		...
		protected void ApplyTransform(SpriteBatch spriteBatch, Matrix transform)
		{
			_previousBatcherTransform = spriteBatch.TransformMatrix;
            spriteBatch.End();
		...
		{
            spriteBatch.End();
            spriteBatch.Begin(_previousBatcherTransform);
		}
