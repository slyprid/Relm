...
namespace Relm.UI.Base
{
	public class Element : ILayout
	{
		...
		public virtual void Draw(SpriteBatch spriteBatch, float parentAlpha) => Validate();
		...
		public bool ClipBegin(SpriteBatch spriteBatch) => ClipBegin(spriteBatch, x, y, width, height);
		...
		/// </summary>
		public bool ClipBegin(SpriteBatch spriteBatch, float x, float y, float width, float height)
		{
			if (width <= 0 || height <= 0)
				return false;

			var tableBounds = RectangleExtensions.FromFloats(x, y, width, height);
			var scissorBounds =
				ScissorStack.CalculateScissors(_stage?.Entity?.Scene?.Camera, spriteBatch.TransformMatrix, tableBounds);
		...
		public void ClipEnd(SpriteBatch spriteBatch)
		{
		...
		{
			if (_debug)
                spriteBatch.DrawHollowRect(x, y, width, height, Color.Red);
		}
