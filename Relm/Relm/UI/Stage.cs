...
namespace Relm.UI
{
	public class Stage
	{
		...
		void DrawDebug()
		{
			...
			{
				var mousePos = ScreenToStageCoordinates(RelmInput.RawMousePosition.ToVector2());
		...
		{
			return Entity != null && !IsFullScreen ? RelmInput.ScaledMousePosition : RelmInput.RawMousePosition.ToVector2();
		}
		...
		public void Update()
		{
			...
			if (RelmInput.Touch.IsConnected && RelmInput.Touch.CurrentTouches.Count > 0)
			{
				UpdateInputTouch();
			}
		}
		...
		void UpdateInputMouse()
		{
			...
			UpdateInputPoint(inputPos, RelmInput.LeftMouseButtonPressed, RelmInput.RightMouseButtonPressed, RelmInput.LeftMouseButtonReleased, RelmInput.RightMouseButtonReleased,
		...
		{
			foreach (var touch in RelmInput.Touch.CurrentTouches)
			{
				...
				if (Entity != null && !IsFullScreen)
					inputPos = RelmInput.ScaledPosition(inputPos);
		...
		void HandleMouseWheel(Element mouseOverElement)
		{
			...
			if (RelmInput.MouseWheelDelta == 0)
				return;
			...
			if (listener != null && listener.OnMouseScrolled(RelmInput.MouseWheelDelta))
				return;
		...
		void UpdateKeyboardState()
		{
			...
			var currentPressedKeys = RelmInput.CurrentKeyboardState.GetPressedKeys();
				...
				if (!_lastPressedKeys.Contains(key))
				{
					...
					if (!InputUtils.IsAltDown())
					{
						...
						if (c.HasValue)
						{
							...
							{
								_repeatKey = key;
								_keyRepeatTimer = RelmGame.Schedule(_keyRepeatTime, true, this, t =>
								{
		...
		void UpdateGamepadState()
		{
			...
			{
				if (RelmInput.GamePads[0].IsButtonPressed(GamepadActionButton) ||
                    (KeyboardEmulatesGamepad && RelmInput.IsKeyPressed(KeyboardActionKey)))
					_gamepadFocusElement.OnActionButtonPressed();
				else if (RelmInput.GamePads[0].IsButtonReleased(GamepadActionButton) ||
                         (KeyboardEmulatesGamepad && RelmInput.IsKeyReleased(KeyboardActionKey)))
					_gamepadFocusElement.OnActionButtonReleased();
			}

			IGamepadFocusable nextElement = null;
			var direction = Direction.None;
			if (RelmInput.GamePads[0].DpadLeftPressed || RelmInput.GamePads[0].IsLeftStickLeftPressed() ||
                (KeyboardEmulatesGamepad && RelmInput.IsKeyPressed(Keys.Left)))
				direction = Direction.Left;
			else if (RelmInput.GamePads[0].DpadRightPressed || RelmInput.GamePads[0].IsLeftStickRightPressed() ||
                     (KeyboardEmulatesGamepad && RelmInput.IsKeyPressed(Keys.Right)))
				direction = Direction.Right;
			else if (RelmInput.GamePads[0].DpadUpPressed || RelmInput.GamePads[0].IsLeftStickUpPressed() ||
                     (KeyboardEmulatesGamepad && RelmInput.IsKeyPressed(Keys.Up)))
				direction = Direction.Up;
			else if (RelmInput.GamePads[0].DpadDownPressed || RelmInput.GamePads[0].IsLeftStickDownPressed() ||
                     (KeyboardEmulatesGamepad && RelmInput.IsKeyPressed(Keys.Down)))
