...
namespace Relm.UI.Widgets
{
	public class SelectBox<T> : Element, IInputListener where T : class
	{
		...
		public override void Layout()
		{
			...
			if (bg != null)
				_prefHeight = System.Math.Max(bg.TopHeight + bg.BottomHeight + font.LineHeight - font.Padding.Bottom * 2f,
					bg.MinHeight);
			else
			...
			for (var i = 0; i < _items.Count; i++)
				maxItemWidth = System.Math.Max(font.MeasureString(_items[i].ToString()).X, maxItemWidth);
			...
			if (_selectBoxList == null || !_selectBoxList.IsScrollingDisabledY())
				listWidth += System.Math.Max(style.ScrollStyle.VScroll != null ? style.ScrollStyle.VScroll.MinWidth : 0,
		...
		public override void Draw(SpriteBatch spriteBatch, float parentAlpha)
		{
			...
			if (selected != null)
			{
				...
				fontColor = ColorExtensions.Create(fontColor, (int)(fontColor.A * parentAlpha));
				spriteBatch.DrawString(font, str, new Vector2(x, y), fontColor);
			}
		}
