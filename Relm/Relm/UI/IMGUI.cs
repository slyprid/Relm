...
#pragma warning disable 0414
		...
		static IMGUI()
		{
			_batcher = new SpriteBatch(RelmGame.GraphicsDevice);
							   ...
							   float elementHeight = ELEMENT_HEIGHT)
		{
			...
			SpriteBatchBitmapFontExtensions.DrawString(_batcher, _font, text, new Vector2(x, y), color, 0, Vector2.Zero, FONT_SCALE, SpriteEffects.None, 0);
		}
		...
		public static void BeginWindow(float x, float y, float width, float height, bool useRawMousePosition = true)
		{
			...
			var mousePos = useRawMousePosition ? RelmInput.RawMousePosition : RelmInput.ScaledMousePosition.ToPoint();
			_mouseInWorldCoords = mousePos - new Point(RelmGame.GraphicsDevice.Viewport.X, RelmGame.GraphicsDevice.Viewport.Y);
		}
		...
		public static bool Button(string text)
		{
			...
			{
				ret = RelmInput.LeftMouseButtonReleased;
				color = RelmInput.LeftMouseButtonDown ? BUTTON_COLOR_DOWN : BUTTON_COLOR_ACTIVE;
			}
